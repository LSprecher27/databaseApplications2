...
SELECT player_id, pts / gp as ppg, gp
FROM player_season_stats
WHERE gp > 0
ORDER BY ppg DESC;
...
SELECT player_id, ROUND(pts / gp) AS ppg, gp
FROM player_season_stats
WHERE gp > 0
ORDER BY ppg DESC;
...
SELECT DISTINCT(SUBSTR(season, 1, 4)) AS season
...
-- Compare your results from queries 4 and 5. What changed? All numbers are rounded whole numbers
...
FROM player_season_stats
WHERE gp = 0;
--There is no player with zero games played

-- =====================================================================
-- VOCABULARY — your words, not the reference sheet's
-- =====================================================================

-- Calculated column: A column created by putting two other columns in a formula


-- Concatenation (||): Joining of text and data